  ...
  async getCalendarEvents(year?: string, month?: string): Promise<any[]> {
    ...
    if (year && month) {
      ...
      const activeRentals = await db.select()
        ...
        .where(
          and(
            ...
            sql`(${rentals.startDate} >= ${startDate} AND ${rentals.startDate} <= ${endDate}) OR (${rentals.endDate} >= ${startDate} AND ${rentals.endDate} <= ${endDate})`
          )
        );
      ...
      rentalEvents = activeRentals.flatMap(rental => {
        ...
        // Evento de entrega
        if (rental.rental.startDate >= startDate && rental.rental.startDate <= endDate) {
          events.push({
            id: `delivery-${rental.rental.id}`,
            title: `Entrega - ${rental.customer.name}`,
            date: rental.rental.startDate,
            time: "09:00",
            type: "delivery",
            status: rental.rental.status === 'confirmado' ? 'scheduled' : 'completed',
            description: `Entrega de ${rental.rental.quantity} cajas ${rental.rental.boxType}`,
            customerName: rental.customer.name,
            driverId: rental.rental.driverId,
            address: rental.rental.deliveryAddress,
        ...
        // Evento de retiro
        if (rental.rental.endDate >= startDate && rental.rental.endDate <= endDate) {
          events.push({
            id: `pickup-${rental.rental.id}`,
            title: `Retiro - ${rental.customer.name}`,
            date: rental.rental.endDate,
            time: "14:00",
            type: "pickup",
            status: rental.rental.status === 'activo' ? 'scheduled' : 'completed',
            description: `Retiro de ${rental.rental.quantity} cajas ${rental.rental.boxType}`,
            customerName: rental.customer.name,
            driverId: rental.rental.driverId,
            address: rental.rental.deliveryAddress,
            isAutoGenerated: true,
    ...
    return [...manualEvents, ...rentalEvents].sort((a, b) => {
